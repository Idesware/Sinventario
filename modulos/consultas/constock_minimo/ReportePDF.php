...
<h3 align="center" ><strong >Productos por Agotarse en <?php echo $sucursal['suc_nombre'];?> </strong></h3>
	<!-- INICIO -->
    ...
    <!-- DETALLE DEL REPORTE -->
		   ...
		   $suc_id= $sucursal['suc_id'];
		   	$sql = sprintf("SELECT * FROM producto INNER JOIN detalle_producto on producto.pro_id = detalle_producto.pro_id INNER JOIN stock on detalle_producto.det_pro_id = stock.det_pro_id WHERE pro_eliminado = 'N' AND det_pro_eliminado = 'N' AND stk_cantidad <= stk_minimo AND suc_id = %s", GetSQLValueString($suc_id, "int"));
		 ...
		 ?>
             ...
             <?php mysql_free_result($query);
              }else{ echo '<div ><h4>No hay productos por agotarse.</h4></div>'; }
            ?>
