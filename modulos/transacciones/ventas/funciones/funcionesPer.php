<?php 
	...
	if($accion=='InsertarCliente'){
		...
		$sql_cliente = sprintf('INSERT INTO cliente (per_id, cli_est, cli_eliminado) VALUES (%s, %s, %s)',
		GetSQLValueString($id, 'text'),
		GetSQLValueString('Activo', 'text'),
		GetSQLValueString('N', 'text'));
		$query_2 = mysql_query($sql_cliente, $conexion_mysql);
		$error = mysql_error();
	...
	if($accion=='Insertar'){
		...
		if($existPer==NULL){
		//Query 1
		$sql = sprintf('INSERT INTO persona (per_nombre, per_documento, per_direccion, per_mail,per_telefono) VALUES (%s, %s, %s, %s, %s)',
		GetSQLValueString($nom, 'text'),
		GetSQLValueString($doc, 'text'),
		GetSQLValueString($dir, 'text'),
		GetSQLValueString($mail, 'text'),
	...
	header("Location: ".$RUTAm."transacciones/ventas/index.php");
